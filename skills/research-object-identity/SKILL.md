...
| orcid | 去 URL 取纯 ID | `https://orcid.org/0000-0002-1825-0097` → `0000-0002-1825-0097` |
...
| CANDIDATE | 疑似同一对象 | 无共享标识符，标题 token 相似度过阈值或作者有重叠 |
| UNRESOLVED | 无法判断 | 信息不足（含空记录列表） |

match_fields 记录一致字段名，conflict_fields 记录冲突字段名，全部是离散字段，供人复核与下游引用。

## 工作流三：建边（link）

`link(a, b, kind, evidence)` 在两个对象之间建边并**双向回填**：

- kind 属于 relations（cites、contradicts、replicates、derived_from）：写入双方 relations，互指对方 object_id；
...
`from_canonical_work(cw)` 把 literature-analysis 的 CanonicalWork 转成 Work 型 ResearchObject 草案：鸭子类型接受实例或同名字段 dict，不 import 对方模块；doi 经 normalize 后入 identifiers，title、authors、year、container 作为信息字段附带，供 resolve 聚合。CanonicalWork 本身与其五个转换函数原样不动。
...
- sources 逐条直传为 source_observations（source、queried_at、status、raw_identifier）；
...
离线冒烟：归一五类标识符，对五态各构造一条 fixture，建边验证双向回填，消费一条迷你回执。
...
assert rid.normalize('orcid', 'https://orcid.org/0000-0002-1825-0097') == '0000-0002-1825-0097'
...
assert b['relations'][0]['target_object_id'] == a['object_id']
